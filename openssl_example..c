...
int main() {
    ...
    EVP_PKEY_copy_parameters(public_key, private_key);
    ...
    encrypted_length = RSA_public_encrypt(strlen(message) + 1, (unsigned char *)message,
                                          encrypted, EVP_PKEY_get1_RSA(public_key), RSA_PKCS1_PADDING);
    fclose(public_key_file);

    // Decrypt the message using the private key
    private_key_file = fopen("private_key.pem", "rb");
    private_key = PEM_read_PrivateKey(private_key_file, NULL, NULL, NULL);
    decrypted_length = RSA_private_decrypt(encrypted_length, encrypted, decrypted,
                                           EVP_PKEY_get1_RSA(private_key), RSA_PKCS1_PADDING);
    fclose(private_key_file);
